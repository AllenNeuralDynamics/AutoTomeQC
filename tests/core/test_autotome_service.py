...
def test_process_fails_file_not_found(service, caplog):
    """Test that non-existent files are caught and logged."""
    service.start()

    # Act
    future = service.process("non_existent_ghost_file.jpg")

    # Assert
    # The service catches FileNotFoundError and logs "Submission Failed"
    assert "Submission Failed" in caplog.text
    assert "File not found" in caplog.text

    # Ensure we didn't try to process
    service.pipeline.process.assert_not_called()

    # Ensure the returned future holds the exception
...
def test_process_handles_exception(service, tmp_path, caplog):
    ...
    # Simulate the pipeline crashing
    service.pipeline.process.side_effect = Exception("YOLO Crashed")

    # Act
    future = service.process(str(dummy_image))

    # Assert
    # Check for the actual log message your code produces
    assert "Submission Failed" in caplog.text
    assert "YOLO Crashed" in caplog.text

    # Service should remain running despite the error
    assert service.running is True

    # Future should contain the exception
    with pytest.raises(Exception, match="YOLO Crashed"):
        future.result()